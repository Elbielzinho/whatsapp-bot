...
const { RemoveBgResult, removeBackgroundFromImageBase64, removeBackgroundmáximoageFile } = require('remove.bg')
...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        const mess = {
            ...
            error: {
                St: '[❗]Envie a imagem com a legenda *!sticker* ou a tag da imagem que foi enviada',
                ...
                Ki: '[❗] Bots não podem emitir administradores de grupo!',
        ...
        const ownerNumber = ["5527997460219@c.us","5527997460219"] // replace with your whatsapp number
        ...
        case '!sgif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    client.reply(from, '[❗] Envie um vídeo com uma legenda *!stickerGif* o máximo é de 10 segundos!', id)
                )
            }
            break
	    case '!stickernobg':
        ...
        case '!donate':
            ...
            break
        case '!tts':
            if (args.length === 1) return client.reply(from, 'Envie o comando *!tts [idioma] [texto]*, exemplo *!tts pt-br Elbielzinho da RedePoseidon*')
	    ...
	    const ttsJp = require('node-gtts')('ja')
            ...
            break
        case '!nulis':
            if (args.length === 1) return client.reply(from, 'Envie o comando *!nulis [texto]*', id)
        ...
        case '!ytmp3':
            ...
            try {
                ...
                if (resp.error) {
                    ...
                    if (Number(filesize.split(' MB')[0]) >= 10.00) return client.reply(from, 'Desculpe, a duração do vídeo excedeu o limite máximo!', id)
                    client.sendFileFromUrl(from, thumb, 'thumb.jpg', `➸ *Título* : ${title}\n➸ *Tamanho do arquivo* : ${filesize}\n\nAguarde alguns minutos para que o processo de envio do arquivo demore alguns minutos.`, id)
        ...
        case '!ytmp4':
            ...
            try {
                ...
                if (ytv.error) {
                    ...
                    if (Number(ytv.filesize.split(' MB')[0]) > 15.00) return client.reply(from, 'Desculpe, a duração do vídeo excedeu o limite máximo!', id)
            ...
            break
        case '!wiki':
            if (args.length === 1) return client.reply(from, 'Envie o comando *!wiki [assunto]*', id)
            ...
            break
        case '!clima':
            if (args.length === 1) return client.reply(from, 'Envie o comando *!clima [lugar]*\nExemplo : *!cuaca país', id)
            ...
            if (weather.error) {
                ...
                client.reply(from, `➸ O lugar : ${weather.result.tempat}\n\n➸ Vento : ${weather.result.angin}\n➸ Clima : ${weather.result.cuaca}\n➸ Descrição : ${weather.result.desk}\n➸ Umidade : ${weather.result.kelembapan}\n➸ Temperatura : ${weather.result.suhu}\n➸ Ar : ${weather.result.udara}`, id)
            }
            break
        case '!fb':
            if (args.length === 1) return client.reply(from, 'Envie o comando *!fb [linkFb]*', id)
            ...
            break
        case '!creator':
            client.sendContact(from, '5527997460219@c.us')
            break
        case '!ig':
            if (args.length === 1) return client.reply(from, 'tangerang *!ig [linkIg]*')
        ...
        case '!nsfw':
            ...
            if (args[1].toLowerCase() === 'enable') {
                ...
                client.reply(from, 'NSWF Comando ativado com sucesso neste grupo! envie o comando *! nsfwMenu * para descobrir o menu', id)
            } else if (args[1].toLowerCase() === 'disable') {
                nsfw_.splice(chat.id, 1)
                fs.writeFileSync('./lib/NSFW.json', JSON.stringify(nsfw_))
                client.reply(from, 'NSFW Command berhasil di nonaktifkan di group ini!', id)
            } else {
                client.reply(from, 'Comando desativado com sucesso neste grupo!', id)
            }
            break
        case '!bemvindo':
            if (!isGroupMsg) return client.reply(from, 'Este comando só pode ser usado em grupos!', id)
            ...
            if (args[1].toLowerCase() === 'enable') {
                ...
                client.reply(from, 'Selecione enable ou disable!', id)
            }
            break
        case '!nsfwmenu':
            if (!isNsfw) return
            ...
            break
        case '!igstalk':
            if (args.length === 1)  return client.reply(from, '*!igStalk @username*', id)
            ...
            const caps = `➸ *Nome* : ${Name}\n➸ *Nome de usuário* : ${Username}\n➸ *Número de Seguidores* : ${Jumlah_Followers}\n➸ *Número de Seguintes* : ${Jumlah_Following}\n➸ *Número de posts* : ${Jumlah_Post}\n➸ *Biodata* : ${Biodata}`
        ...
        case '!infomapa':
            ...
            const hasil = `*${waktu}*\n📍 *Localização* : *${lokasi}*\n〽️ *Profundidade* : *${kedalaman}*\n💢 *Magnitude* : *${magnitude}*\n🔘 *Potência* : *${potensi}*\n📍 *Coordenada* : *${koordinat}*`
            client.sendFileFromUrl(from, map, 'shakemap.jpg', hasil, id)
            break
        case '!anime':
            if (args.length === 1) return client.reply(from, 'Envie o comando *!anime [consulta]*', id)
            ...
            const res_animek = `Título: *${animek.title}*\n\n${animek.info}\n\nSinopse: ${animek.sinopsis}\n\nLink Download:\n${animek.link_dl}`
        ...
        case '!nh':
            ...
            if (args.length === 2) {
                ...
                if (cek === true)  {
                    ...
                    client.reply(from, '[❗] Código nuClear incorreto!')
                }
            } else {
                client.reply(from, '[ ERRADO ] Enviar pedidos *!nh [nuClear]* por exemplo enviar comando *!readme*')
            }
        	break
        case '!brainly':
            if (args.length >= 2){
            ...
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                .then(resolt => {
                	if (resolt.docs && resolt.docs.length <= 0) {
                		client.reply(from, 'Desculpe, não sei que anime é este', id)
                	}
                    ...
                    teks += `➸ *Título Japanese* : ${title}\n➸ *Título chinese* : ${title_chinese}\n➸ *Título Romaji* : ${title_romaji}\n➸ *Título Inglês* : ${title_english}\n`
                    teks += `➸ *Ecchi* : ${is_adult}\n`
        ...
        case '!quotemaker':
            ...
            if (arg.length >= 4) {
                ...
                client.reply(from, 'Usa: \n!quotemaker |texto|marca d'água|tema\n\nEx :\n!quotemaker |isto é um exemplo|bicit|random', id)
            }
            break
        case '!linkgroup':
            if (!isBotGroupAdmins) return client.reply(from, 'Este comando só pode ser usado quando o bot é um administrador', id)
            if (isGroupMsg) {
            ...
            break
        case '!bc':
            if (!isOwner) return client.reply(from, 'Este comando é apenas para proprietários de bot!', id)
            let msg = body.slice(4)
            const chatz = await client.getAllChatIds()
            for (let ids of chatz) {
                var cvk = await client.getChatById(ids)
                if (!cvk.isReadOnly) await client.sendText(ids, `[ *ATENÇÃO - AVISO_BOT* ]\n\n${msg}`)
            }
        ...
        case '!f-s':
            ...
            await client.sendTextWithMentions(from, `Fundador-Supremo : @${Owner_}`)
            break
        case '!tagall':
            if (!isGroupMsg) return client.reply(from, 'Este comando só pode ser usado em grupos!', id)
        ...
        case '!kickall':
            ...
            if (!isGroupOwner) return client.reply(from, 'Este comando só pode ser usado pelo proprietário do grupo', id)
            if (!isBotGroupAdmins) return client.reply(from, 'Este comando só pode ser usado quando o bot é um administrador :)', id)
            const allMem = await client.getGroupMembers(groupId)
            for (let i = 0; i < allMem.length; i++) {
                if (groupAdmins.includes(allMem[i].id)) {
                    console.log('Upss, este é o grupo de administraçãop')
                } else {
                    await client.removeParticipant(groupId, allMem[i].id)
                }
            }
            client.reply(from, 'Sucesso ao kicka todos os membros', id)
            break
        case '!leaveall':
            if (!isOwner) return client.reply(from, 'Este comando é apenas para proprietários de bot', id)
            ...
            for (let gclist of allGroups) {
                await client.sendText(gclist.contact.id, `O bot está limpando, o bate-papo total está ativo : ${allChats.length}`)
                await client.leaveGroup(gclist.contact.id)
            }
            client.reply(from, 'Sucesso deixa todo o grupo!', id)
            break
        case '!clearall':
            if (!isOwner) return client.reply(from, 'Este comando é apenas para proprietários de bot', id)
            ...
            client.reply(from, 'Sucesso limpar todo o bate-papo!', id)
            break
        case '!add':
            const orang = args[1]
            if (!isGroupMsg) return client.reply(from, 'Este recurso só pode ser usado em grupos', id)
            if (args.length === 1) return client.reply(from, 'Para usar este recurso, envie um comando *!add* 55DDD....', id)
        ...
        case '!kick':
            ...
            if (!isGroupAdmins) return client.reply(from, 'Este comando só pode ser usado por administradores do grupo', id)
            if (!isBotGroupAdmins) return client.reply(from, 'Este comando só pode ser usado quando o bot é um administrador :)', id)
            if (mentionedJidList.length === 0) return client.reply(from, 'Para usar este comando, envie um comando *!kick* @membro', id)
            await client.sendText(from, `Kick aceito, kickando o:\n${mentionedJidList.join('\n')}`)
        ...
        case '!promote':
            if (!isGroupMsg) return client.reply(from, 'Este recurso só pode ser usado em grupos', id)
            if (!isGroupAdmins) return client.reply(from, 'Este recurso só pode ser usado por administradores do grupo', id)
            if (!isBotGroupAdmins) return client.reply(from, 'Este recurso só pode ser usado quando o bot é um administrador :)', id)
            if (mentionedJidList.length === 0) return client.reply(from, 'Para usar este recurso, envie um comando *!promote* @membro', id)
            ...
            await client.sendTextWithMentions(from, `Pedidos aceitos, adicionados @${mentionedJidList[0]} como administrador.`)
            break
        case '!demote':
            if (!isGroupMsg) return client.reply(from, 'Este recurso só pode ser usado em grupos', id)
            if (!isGroupAdmins) return client.reply(from, 'Este recurso só pode ser usado por administradores do grupo', id)
            if (!isBotGroupAdmins) return client.reply(from, 'Este recurso só pode ser usado quando o bot é um administrador', id)
            if (mentionedJidList.length === 0) return client.reply(from, 'Para usar este recurso, envie um comando *!demote* @admin', id)
            ...
            await client.sendTextWithMentions(from, `Pedido aceito, remover admin do @${mentionedJidList[0]}.`)
            break
        case '!join':
            //return client.reply(from, 'Jika ingin meng-invite bot ke group anda, silahkan izin ke wa.me/6285892766102', id)
            if (args.length < 2) return client.reply(from, 'Envie o comando *!join LinkDoGrupo key*\nA key você pode obtê-la com apenas uma doação de 5k', id)
            const link = args[1]
            const key = args[2]
            const tGr = await client.getAllGroups()
            const minMem = 30
            const isLink = link.match(/(https:\/\/chat.whatsapp.com)/gi)
            if (key !== 'lGjYt4zA5SQlTDx9z9Ca') return client.reply(from, '*key* errada! converse com o proprietário do bot para obter uma chave válida', id)
            const check = await client.inviteInfo(link)
            if (!isLink) return client.reply(from, 'Aqui está um link? 👊🤬', id)
            if (tGr.length > 15) return client.reply(from, 'Desculpe, o número de grupos ja está no limite!', id)
            if (check.size < minMem) return client.reply(from, 'Membros do grupo não superiores a 30, os bots não podem entrar', id)
            if (check.status === 200) {
                await client.joinGroupViaLink(link).then(() => client.reply(from, 'Bot vai entrar em breve'))
            } else {
        ...
        case '!delete':
            if (!isGroupMsg) return client.reply(from, 'Este recurso só pode ser usado em grupos', id)
            if (!isGroupAdmins) return client.reply(from, 'Este recurso só pode ser usado por administradores do grupo', id)
            if (!quotedMsg) return client.reply(from, 'Errado !!, Envie o comando *!delete [MarqueOBot]*', id)
        ...
        case '!getses':
            ...
            break
        case '!lirik':
            if (args.length == 1) return client.reply(from, 'Envie o comando *!lirik [opcional]*, Exemplo *!lirik aku bukan boneka*', id)
            const lagu = body.slice(7)
            const lirik = await liriklagu(lagu)
            client.reply(from, lirik, id)
            break
        case '!chord':
            if (args.length === 1) return client.reply(from, 'Envie o comando *!chord [consulta]*, exemplo *!chord aku bukan boneka*', id)
        ...
        case '!listblock':
            ...
            break
        case '!jadwalshalat':
            if (args.length === 1) return client.reply(from, '[❗] Envie o comando *!jadwalShalat [daerah]*\nexemplo : *!jadwalShalat Tangerang*\nPara listar a área, envie pedidos *!listDaerah*')
        ...
        case '!listchannel':
            ...
            break
        case '!jadwaltv':
            if (args.length === 1) return client.reply(from, 'Envie o comando *!jadwalTv [canal]*', id)
        ...
        case '!waifu':
            ...
            client.sendFileFromUrl(from, waifu.image, 'Waifu.jpg', `➸ Nome : ${waifu.name}\n➸ Descrição : ${waifu.desc}\n\n➸ Fonte : ${waifu.source}`, id)
            break
        case '!husbu':
        ...
        case '!url2img':
            ...
            if (args.length === 1) return client.reply(from, 'Envie o comando *!url2img [web]*\nExemplo: *!url2img https://google.com*', id)
